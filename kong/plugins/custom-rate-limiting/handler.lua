...
local policies = require "kong.plugins.custom-rate-limiting.policies"
...
local EXPIRATION = require "kong.plugins.custom-rate-limiting.expiration"

local IPMATCHER_COUNT = 512
local IPMATCHER_TTL   = 3600
local ipmatcher_cache = lrucache.new(IPMATCHER_COUNT)
...
local function is_ip_whitelisted(white_listed_ips, ip)
  if not white_listed_ips or #white_listed_ips == 0 then
    return false
  end
  ...
  -- Create a string key from the IP list for caching
  -- Sort IPs to ensure consistent key generation
  local sorted_ips = {}
  for i = 1, #white_listed_ips do
    sorted_ips[i] = white_listed_ips[i]
  end
  table.sort(sorted_ips)
  local cache_key = table.concat(sorted_ips, ",")

  local matcher, err

  -- Use cache to avoid recreating matcher for each request
  matcher = ipmatcher_cache:get(cache_key)
  if not matcher then
    matcher, err = ipmatcher.new(white_listed_ips)
    if err then
      kong.log.err("failed to create ipmatcher instance: ", err)
      return false
    end

    ipmatcher_cache:set(cache_key, matcher, IPMATCHER_TTL)
  end

  local is_match, match_err = matcher:match(ip)
  if match_err then
    kong.log.err("failed to match IP address: ", match_err)
    return false
  end
...
function RateLimitingHandler:access(conf)
  ...
  local white_listed_ips = conf.white_listed_ips
  if white_listed_ips and #white_listed_ips > 0 then
    local client_ip = kong.client.get_forwarded_ip()
    if client_ip and is_ip_whitelisted(white_listed_ips, client_ip) then
      -- IP is whitelisted, skip rate limiting
      return
    end
  end
