...
describe("Plugin: rate-limiting (schema)", function()
  ...
  describe("white_listed_ips", function()
    it("proper config validates with white_listed_ips (single IP)", function()
      local config = {
        second = 10,
    ...
    it("proper config validates with white_listed_ips (multiple IPs)", function()
      local config = {
        second = 10,
        white_listed_ips = { "127.0.0.1", "192.168.1.1", "10.0.0.1" }
      }
      local ok, _, err = v(config, schema_def)
      assert.truthy(ok)
      assert.is_nil(err)
    ...
    it("proper config validates with white_listed_ips (CIDR ranges)", function()
      local config = {
        second = 10,
        white_listed_ips = { "127.0.0.1", "192.168.0.0/16", "10.0.0.0/8" }
      }
      local ok, _, err = v(config, schema_def)
      assert.truthy(ok)
      assert.is_nil(err)
    ...
    it("proper config validates with empty white_listed_ips", function()
      local config = {
        second = 10,
        white_listed_ips = {}
      }
    ...
    it("rejects invalid IP in white_listed_ips", function()
      local config = {
        second = 10,
        white_listed_ips = { "invalid-ip" }
      }
      local ok, err = v(config, schema_def)
      assert.falsy(ok)
      assert.matches("invalid ip or cidr range", err.config.white_listed_ips[1]:lower())
    end)

    it("rejects invalid CIDR in white_listed_ips", function()
      local config = {
        second = 10,
        white_listed_ips = { "192.168.1.1/invalid" }
      }
      local ok, err = v(config, schema_def)
      assert.falsy(ok)
      assert.matches("invalid ip or cidr range", err.config.white_listed_ips[1]:lower())
